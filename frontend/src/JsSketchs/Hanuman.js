...
const p5SketchHanuman = (p ,theme, weatherData) => {
    ...
    const updateWeatherData = (weatherData) => {
        ...
        if (weatherData) {
            ...
            windSpeed = weatherData.wind.speed;//1.03-4.25
    ...
    function f1(x, y, n = 0) {
        ...
        if(theme === 'dark'){
            ...
            const my = p.map(p.mouseY,0,p.height,1.5,0.9);   
            if (d < Math.floor(windSpeed) && n<18) {
                return Math.floor(f1(x * x + y * y - a * x / d*my, my*Math.tan(b*x*y)-1/mx, n + 1));
            } else {
                // 
                return cloudsAll < 50 ? (Math.cos(n / 18 + Math.sqrt(x/t)) + 1) * mx/traslucid : (Math.cos(n / 10) + 1) * 28;
            }
        }else {
            a = 1/4;
            b = 1/9;           
            const mx = p.map(p.mouseX, 0, p.width, 4.25, 8.5);  //2.5 ***        
            const my = p.map(p.mouseY,0,p.height,0.9,2.6);
            const smx  = 41.3;
            const smy = 0.2
            ...
            if (d < Math.floor(windSpeed) && n < 8) {
             ...
             return cloudsAll < 50 ?(Math.sin(n / scalar + Math.sqrt(x * my)) + Math.tan(ti/a)) * fact / (2.3*mx):
                (Math.sin(n /(Math.floor(0.8/smy)) + Math.sqrt(x)) + 1) * Math.pow(smx / traslucid*my,0.3);
            } 
        }    
    }
    function fz(x, y, n = 0) {
